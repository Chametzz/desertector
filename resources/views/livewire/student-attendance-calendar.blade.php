<div class="space-y-6">
    <flux:card>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <flux:select wire:model.live="student_id" label="Estudiante" placeholder="Selecciona un alumno">
                @foreach ($students as $student)
                <flux:select.option value="{{ $student->id }}">{{ $student->person->full_name ?? 'Sin nombre' }}
                </flux:select.option>
                @endforeach
            </flux:select>

            <flux:select wire:model.live="subject_id" label="Materia" placeholder="Selecciona materia">
        ...
        <div
            ...
            @for ($i = 1; $i < $firstDayOfMonth; $i++) <div
                class="bg-zinc-50 dark:bg-zinc-900/50 h-24 border border-zinc-100 dark:border-zinc-800">
        </div>
        @endfor

        @for ($day = 1; $day <= $daysInMonth; $day++) @php $absence=$absences[$day] ?? null;
            $hasAbsence=!is_null($absence); $isJustified=$hasAbsence && $absence->is_justified;
            ...
            <div
                ...
                <div class="flex flex-col items-center justify-center h-full pointer-events-none">
                    ...
                    <span class="text-[10px] mt-1 text-gray-500 dark:text-gray-400 truncate max-w-[90%] px-1">
                        {{ Str::limit($absence->justification_reason, 15) }}
                    </span>
                    @endif
                </div>

                <!-- Botón para justificar (solo visible si hay ausencia) -->
...
<div class="mt-4 p-3 bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300 rounded-lg text-sm">
    {{ session('message') }}
</div>
@endif
...
<!-- Modal de justificación simple -->
@if ($showJustifyModal)
<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" wire:key="justify-modal">
